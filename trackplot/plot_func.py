    ...
    for transcript in data:
        ...
        if transcript.transcript or transcript.transcript_id:
            ...
            if not transcript.transcript:
                logger.warning(
                    f"there is not transcript_name, using transcript_id instead"
                )
        ...
        for ind, exon in enumerate(transcript.exons):
            s, e, strand = (
                region.relative(exon.start),
                region.relative(exon.end),
                exon.strand,
            )
        ...
        if plot_domain and obj.domain and transcript.transcript_id in obj.domain.pep:
            ...
            for sub_current_domain in current_domains:
                ...
                for sub_exon in sub_current_domain.exons:
                    ...
                    intron_relative_s = (
                        intron_relative_s if intron_relative_s >= 0 else 0
                    )
                ...
                if show_id:
                    ax.text(
                        x=-1,
                        y=y_loc - 0.125,
                        s=f"{sub_current_domain.gene}|{transcript.transcript_id}",
                        fontsize=font_size / 2,
                        ha="right",
                    )
                else:
                    ax.text(
                        x=-1,
                        y=y_loc - 0.125,
                        s=f"{sub_current_domain.gene}|{transcript.transcript}",
                        fontsize=font_size / 2,
                        ha="right",
                    )
...
def precompute_y_limits(
    ...
    **kwargs,
    ...
    try:
        jxns = data.junctions_dict(False)
    ...
    if max_used_y_val is None:
        if isinstance(data, dict):
            for k, v in data.items():
                max_used_y_val = max(
                    v.plus.max(), max_used_y_val if max_used_y_val else 0
                )
        else:
...
def plot_density(
    ...
    **kwargs,
    ...
    if (
        obj and obj.log_trans and obj.log_trans.isdigit() and int(obj.log_trans) > 0
    ):  # in ["2", "10"]:
    ...
    min_used_y_val = 0  # AD
    fixed_max_used_y, fixed_min_used_y = (
        max_used_y_val is not None,
        min_used_y_val is not None,
    )
    if max_used_y_val is None:
        if isinstance(data, dict):
            for k, v in data.items():
    ...
    if jxns:
        ...
        for jxn_idx, jxn in enumerate(jxns_sorted_list):
            ...
            if (
                ...
                logger.debug(
                    f"junction {jxn} of {y_label} is is out of plotting region, skip"
                )
                continue
        ...
        for c_ind_list in obj.get_index():
            ...
            for c_ind in c_ind_list:
                ...
                for intron in c_data.introns:
                    s, e, strand = (
                        region.relative(intron.start),
                        region.relative(intron.end),
                        intron.strand,
                    )
                    ...
                    s = 0 if s < 0 else s
                    e = len(region) - 1 if e >= len(region) else e
                    intron_sites = [graph_coords[s], graph_coords[e]]
                    patches.append(
                        plt.Rectangle(
                ...
                for feature in c_data.features:
                    ...
                    s, e, strand = (
                        region.relative(feature.start),
                        region.relative(feature.end),
                        feature.strand,
                    )
                    is_site = False
