...
class CdsProtein(GenomicLoci):
    ...
    __slots__ = ["proxy", "timeout", "cds", "pep", "gene_id", "transcript_id"]
    ...
    def __add__(self, other):
        assert isinstance(other, CdsProtein), (
            "only CdsProtein and CdsProtein could be added"
        )
    ...
    def __re_iter_gtf__(
        ...
        for line in relevant_exon_iterator:
            ...
            try:
                if len(boundary_for_iter) == len(gene_id) and line.end > max(
                    boundary_for_iter
                ):
                    break
                ...
                if line.transcript_id in transcript_id and line.feature == "CDS":
        ...
        if len(cds_dict) != 0:
            for current_trans_id in cds_dict.keys():
                ...
                cds_contain[current_trans_id] = Transcript(
                    ...
                    if "gene_name" in current_exon_list[0].keys()
                    else "",
                    gene_id=current_exon_list[0].gene_id
                    if "gene_id" in current_exon_list[0].keys()
                    else "",
                    transcript=current_exon_list[0].transcript_name
                    if "transcript_name" in current_exon_list[0].keys()
                    else "",
                    transcript_id=current_exon_list[0].transcript_id
                    if "transcript_id" in current_exon_list[0].keys()
                    else "",
                )
        ...
        for current_transcript_id in self.cds.keys():
            ...
            for domain in current_pep.domain:
                ...
                pep_genomic_coord = coord_mapper.pep_to_cds(
                    int(domain.begin), int(domain.end)
                )
            ...
            for domain_unique_id, domain_list in domain_res.items():
                start_site = min([min(map(lambda x: x.start, i)) for i in domain_list])
