    ...
    try:
        if category == "density":
            for idx, line in __read_iter__(infile):
                ...
                if len(line) < 3:
                    yield FileList(
                        path=path, category=cat, color=COLORS[idx % len(COLORS)]
                    )
                elif len(line) < 4:
                    yield FileList(
                        path=path,
                    ...
                    yield FileList(
                        path=path, category=cat, color=line[3], label=line[2]
                    )
                elif len(line) < 6:
                    yield FileList(
                        path=path,
            ...
            for idx, line in __read_iter__(infile):
                ...
                if len(line) < 3:
                    ...
                    yield FileList(
                        path=path,
                        category=cat,
                        color=COLORMAP[len(groups) % len(COLORMAP)],
                        group=line[2],
                    )
                else:
                    groups[line[2]] = 0
                    ...
                    yield FileList(
                        ...
                        color=line[3],
                        group=line[2],
                        library=line[4] if len(line) > 4 else "fru",
                        depth=depth,
                    )
        elif category == "line":
            groups = {}
            for idx, line in __read_iter__(infile):
                ...
                if len(line) < 3:
                    ...
                    if len(line) < 4:
                        yield FileList(
                            path=path, category=cat, color=grp_color, group=line[2]
                        )
                    elif len(line) < 5:
                        yield FileList(
                            path=path,
            ...
            for idx, line in __read_iter__(infile):
                ...
                if len(line) < 3:
                    ...
                    yield FileList(
                        path=path, category=cat, label=line[2], depth=default_depth
                    )
                elif len(line) < 5:
                    yield FileList(
                        path=path,
...
_CATEGORY_HANDLERS = {
    ...
    "line": lambda p, k, kw, barcodes, sc_colors, size_factors, region: _handle_line(
        p, kw["line"], kw, barcodes, sc_colors
    ),
    "igv": lambda p, k, kw, barcodes, sc_colors, size_factors, region: _handle_igv(
        p, kw["igv"], kw
    ),
    "hic": lambda p, k, kw, barcodes, sc_colors, size_factors, region: _handle_hic(
        p, kw["hic"], kw
    ),
    ...
    "motif": lambda p, k, kw, barcodes, sc_colors, size_factors, region: _handle_motif(
        p, kw, region
    ),
}
...
@click.option("-e", "--event", type=click.STRING, help="Event range eg: chr1:100-200:+")
...
@optgroup.option(
    ...
    type=click.BOOL,
    is_flag=True,
    help="Group by cell types in density/line plot",
)
@optgroup.option(
    "--remove-duplicate-umi",
    type=click.BOOL,
    is_flag=True,
    help="Drop duplicated UMIs by barcode",
)
@optgroup.option(
    "--normalize-format",
    type=click.Choice(NORMALIZATION),
    default="count",
    help="The normalize format for bam file",
    show_default=True,
)
...
@optgroup.option(
    ...
    default=1,
    type=float,
    help="The height of single subplot, default adjust image height by content",
    show_default=True,
)
@optgroup.option(
    "--width",
    ...
    help="The width of output file, default adjust image width by content",
    show_default=True,
)
@optgroup.option(
    "--backend",
...
@optgroup.option(
    ...
    is_flag=True,
    show_default=True,
    help="Whether show gene id or gene name",
)
@optgroup.option(
    "--show-exon-id",
    is_flag=True,
...
@optgroup.option(
    ...
    type=click.STRING,
    show_default=True,
    help="The color of exons",
)
@optgroup.option(
    "--intron-scale",
    type=click.FLOAT,
    default=0.5,
    help="The scale of intron",
    show_default=True,
)
@optgroup.option(
    "--exon-scale",
...
@optgroup.option(
    "--only-customized-junction",
    is_flag=True,
    show_default=True,
    help="Only used customized junctions.",
)
@optgroup.option(
    "-t",
    "--threshold",
...
@optgroup.option(
    ...
    type=click.BOOL,
    show_default=True,
    help="Whether to draw density plot by strand",
)
@optgroup.option(
    "--show-site",
    is_flag=True,
    type=click.BOOL,
    show_default=True,
    help="Whether to draw additional site plot",
)
@optgroup.option(
    "--site-strand",
...
@optgroup.option(
    ...
    type=click.STRING,
    default=None,
    help="The junction id for including, chr1:1-100",
    show_default=True,
)
@optgroup.option(
    "--show-junction-num",
...
@optgroup.option(
    "--fill-step",
    type=click.Choice(["pre", "post", "mid"]),
    default="post",
    show_default=True,
    help="""
...
@optgroup.option(
    ...
    is_flag=True,
    type=click.BOOL,
    help="Whether to hide legend",
)
@optgroup.option(
    "--legend-position",
    default="upper right",
    type=click.STRING,
    help="The legend position",
)
@optgroup.option(
    "--legend-ncol",
    default=0,
...
@optgroup.option(
    ...
    is_flag=True,
    show_default=True,
    help="Enable clustering of the heatmap",
)
@optgroup.option(
    "--clustering-method",
    type=click.Choice(CLUSTERING_METHOD),
    default="ward",
    show_default=True,
    help="The clustering method for heatmap",
)
@optgroup.option(
    "--distance-metric",
    type=click.Choice(DISTANCE_METRIC),
    default="euclidean",
    show_default=True,
    help="The distance metric for heatmap",
)
@optgroup.option(
    "--heatmap-scale",
    is_flag=True,
    show_default=True,
    help="Do scale on heatmap matrix.",
)
@optgroup.option(
    "--heatmap-vmin",
    type=click.INT,
...
@optgroup.option(
    ...
    is_flag=True,
    show_default=True,
    help="Show row names of heatmap",
)
@optgroup.option(
    "--sc-heatmap-height-ratio",
    type=float,
    default=0.2,
    show_default=True,
...
@optgroup.option(
    ...
    type=click.STRING,
    show_default=True,
    help="The highlight regions: 100-200:300-400",
)
...
@optgroup.option(
    ...
    type=click.STRING,
    default="",
    help="The region of motif to plot in start-end format",
    show_default=True,
)
@optgroup.option(
    "--motif-width",
    type=click.FLOAT,
    default=0.8,
    help="The width of ATCG characters",
    show_default=True,
)
...
@optgroup.option(
    ...
    type=click.FLOAT,
    default=0,
    help="The distance between transcript label and transcript line",
    show_default=True,
)
@optgroup.option(
    "--annotation-scale",
    type=click.FLOAT,
    default=0.25,
    help="The size of annotation plot in final plot",
    show_default=True,
)
@optgroup.option(
    "--stroke-scale",
    type=click.FLOAT,
    default=0.25,
    help="The size of stroke plot in final image",
    show_default=True,
)
...
@optgroup.option(
    ...
    type=click.BOOL,
    is_flag=True,
    help="Start web ui instead of running in cmd mode",
)
@optgroup.option(
    "--host", type=click.STRING, default="localhost", help="The ip address binding to"
)
...
def main(**kwargs):
    ...
    if (kwargs["domain"] or kwargs["local_domain"]) and kwargs[
        "backend"
    ].lower() != "cairo":
        logger.debug(
            f"{kwargs['backend']} backend may have problems with small domain, "
            f"if there is any please try cairo backend instead."
        )

    if kwargs["raster"] and kwargs["heatmap"] and kwargs["backend"].lower() == "cairo":
        logger.debug(
            f"{kwargs['backend']} backend may have problems with rasterized heatmap, "
    ...
    if kwargs["included_junctions"] is not None:
        included_junctions = set(
            [sj.strip() for sj in kwargs["included_junctions"].split(",")]
        )
    else:
        included_junctions = {}

    p = Plot(
        logfile=kwargs["logfile"], font_family=kwargs["font"], backend=kwargs["backend"]
    )
        ...
        if key in IMAGE_TYPE and kwargs[key] and os.path.exists(kwargs[key]):
            if key == "annotation":
                ...
                _CATEGORY_HANDLERS[key](
                    p, key, kwargs, barcodes, sc_colors, size_factors, region
                )
        elif key == "focus":
