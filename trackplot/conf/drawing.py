...
def draw_line_polar(
    svg_file, x_origin, y_origin, length, angle, thickness=10, color=RGB(0, 0, 0)
):
...
def draw_bezier(
    svg_file, x1, y1, x2, y2, controlX, controlY, color=RGB(0, 0, 0), thickness=1
):
...
def draw_multiline_text(svg_file, label, x, y, size, color=RGB(0, 0, 0)):
    ...
    for i in range(len(words_list)):
        if i == 0:
            ...
            svg_file.write(
                '<tspan x="{0}" dy="{1}">{2}</tspan>'.format(x, size, words_list[i])
            )
    svg_file.write("</text>")
