...
Options:
  ...
  Common input files configuration:
    ...
    --umi-tag TEXT                The default UMI barcode tag label  [default:
                                  UB]
    -p, --process INTEGER RANGE   How many cpu to use  [1<=x<=8]
  ...
  IGV settings:
    ...
    --del-ratio-ignore FLOAT RANGE
                                  ...
                                  [0.0<=x<=1.0]
    --igv-height-scale FLOAT      Relative IGV/read-track height scaled by
                                  read count. Use larger values, e.g. 0.6-1.0,
                                  to make individual isoforms easier to
                                  inspect.
  HiC settings:
  ...
  Overall settings:
    ...
    --title TEXT                  Title
    --no-title                    Disable the plot title entirely
    --no-region-text              Disable the --event region text on the
                                  x-axis ruler
    --junctions-on-top            Force all junction arcs to be drawn above
                                  the density plot (only effective when
                                  --density-by-strand is not set)
    --font TEXT                   Fonts
  Web settings:
